...
sequenceDiagram
  ...
  Owner->>Peer: close after idle or terminal loss
```
...
When changing ownership or scheduling, start in `internal/jetkvm/device_owner.go` and `scheduling.go`, then trace `Manager.withOperation` in `manager.go`. Preserve: no connection attempt after all setup waiters leave; no reconnect after incomplete cleanup; cancellation before dispatch is `not_sent`; and stale completions cannot alter a replacement generation. `device_owner_test.go` names the corresponding cases, while `scheduling_test.go` covers interleaving, cancellation, and capture frame ownership. Run `go test ./internal/jetkvm`; add `make race` when changing owner, scheduler, decoder, or video concurrency.
