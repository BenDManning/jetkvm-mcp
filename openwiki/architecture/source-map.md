...
| MCP registration and outputs | `internal/mcpserver/server.go`, `controls.go` | official Go SDK, JSON Schema; `Device.ReleaseSession` bridges the local lifecycle tool | `manifest_contract_test.go`, `release_session_test.go`, `typed_results_test.go`, `input_contract_test.go` |
...
| Appliance manager and ownership | `internal/jetkvm/manager.go`, `device_owner.go`, `scheduling.go` | `mcpserver.Device`; managed resident generation and explicit local release per device | `release_session_test.go`, `device_owner_test.go`, `scheduling_test.go`, `manager_test.go`, `admission_test.go` |
