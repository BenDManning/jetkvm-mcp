...
The topology keeps ownership isolated per configured device rather than sharing a process-wide appliance connection. A healthy generation can serve multiple operations and is closed only when the owner becomes idle, the generation ends, or shutdown occurs. Authentication cookies and WebRTC resources remain generation-scoped, while each dispatched worker keeps its own cancellation context. Read [session lifecycle](../runtime/sessions-and-device-protocol.md) for the owner state machine and connection sequence; [public surface](../mcp/public-surface.md) dispatches handlers through that boundary.
...
| Change session ownership, idle reuse, or scheduling | `internal/jetkvm/device_owner.go`, `scheduling.go` | `device_owner_test.go`, `scheduling_test.go` |
