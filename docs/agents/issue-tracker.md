...
Every blocker addition, removal, or replacement on an existing child must use that child's same `ticket-state` lease. The winner re-reads the ticket and proceeds only while it is open and unassigned; never rewire an assigned child. Apply the change through the map's recorded blocker backend, then re-read and verify the complete blocker set before releasing the lease. If the ticket is assigned or another attempt wins, release any losing attempt and stop without changing blockers. Holding this lease from the claimant's final blocker check through assignment prevents a blocker rewrite from interleaving with a claim.
