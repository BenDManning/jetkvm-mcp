...
- Automated tests do not qualify physical compatibility. The repository's configured development JetKVM and attached disposable fixture are available under standing owner authorization: use them when device or protocol behavior needs physical evidence, and stop a mutation sequence on any `outcome: unknown` until state is independently established.
- Keep generated artifacts out of source control. Device aliases, endpoints, firmware versions, and transport observations are ordinary engineering data rather than secrets.
...
The OpenWiki GitHub Actions workflow refreshes the repository wiki after relevant changes reach `main` or on manual dispatch. Do not hand-edit generated OpenWiki pages unless explicitly asked; prefer updating source code/docs and letting OpenWiki regenerate.
